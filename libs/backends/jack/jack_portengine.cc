...
// #define JACK_SERVER_CALL(expr) { std::cerr << "JACK SERVER CALL: " << DEBUG_THREAD_SELF << '/' << pthread_name() << ' ' << #expr << std::endl; PBD::Mutex::Lock lm (server_call_mutex); expr; }
#define JACK_SERVER_CALL(expr) { PBD::Mutex::Lock lm (server_call_mutex); expr; }
